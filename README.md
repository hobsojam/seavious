...
**Song form** (decided after playtests found the stage music repetitive —
the old loop was 8 bars ≈ 12.6s, heard ~18 times per stage): the stage
gameplay themes are compiled as a full form, `intro | A | A' | B | break |
B' | A'` (44 bars, ~69s loop body), using the XM **order table** and
**restart position** — the drums+bass intro plays exactly once, then the
body loops from A (raylib's jar_xm honors the restart position natively;
repeated sections dedup to order-table references, so recapitulation is
nearly free). Variation comes from *arrangement*, not just length: A'/B'
sections add a harmony channel (a 2-row same-pitch echo of the lead on a
softer 25% pulse — the classic tracker delay illusion), and the break
strips to half-time drums under the walking bass. Each stage's music is
one bass line plus two 8-bar tunes (A and B) poured into the shared form —
stage 1: "anthem" + "spray"; stage 2: "strait" + "undertow" (D Dorian).
Key changes were considered and deliberately deferred. Boss and menu
themes intentionally stay tight 8-bar loops: repetition reads as intensity
in a fight, and nobody lingers on the title screen.

Current tracks (first pass, generated by `tools/gen-music-xm.py` since
OpenMPT isn't reachable from the WSL dev environment; auditioned and
confirmed playable): `stage1_theme_a.xm` (song form over "anthem" — long
held notes, mostly stepwise — and "spray" — call-and-response with
offbeat pushes) is the stage 1 gameplay theme. `stage2_theme_a.xm` (song
form over "strait" and "undertow", D Dorian) is the stage 2 gameplay
theme, with `boss2_theme_a.xm` ("gate") for its fortress-atoll boss.
`boss1_theme_b.xm` ("hammer" — relentless straight-8ths arpeggio
...
tracks via `tools/gen-music-xm.py` into a temp dir, validates each file
