...
theme; `stage1_drums_bass.xm` (the backing-only template — the tune
dropping out reads as deflation) plays over the game-over screen;
`stage1_theme_b.xm` ("driver" — a syncopated arpeggio cell transposed
through the chords) is reserved for future modal screens (menus,
high-score entry) and isn't wired in yet.
