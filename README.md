...
interleave rather than wall), the player-facing hull section fires
casemate-style while the shielded one waits its turn, the mortar lobs
...
clamps to it), so only the amber hull section on the side facing you
can be torpedoed; the turn at each patrol end swings the other one
around. Gun down its magenta AA pods anytime, destroy both hull
sections to tear the breach open and expose the white-hot core (weak
