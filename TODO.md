...
- [ ] Implement air roster: Interceptor, Gunship (Skimmer Drone and its
      Wing Formations are done) — sprites + behavior numbers are designed
      (see README/Art) and nothing is blocked anymore (the lives/damage
      system exists); their stage glyphs (`i`/`G`) currently compile into
      the table but are skipped
