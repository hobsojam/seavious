...
Tune A ("anthem"): long held notes, mostly stepwise, chord tones on strong
beats — a broad heroic line that leaves space over the busy bass. Ends on
the leading tone (G#) so the loop restart resolves onto A.

Tune B ("driver"): one syncopated arpeggio cell (with an off-beat push on
the "2-and-a-half") transposed through each chord — denser and more
chiptune-idiomatic, closer to the classic shooter energy. Bar 8 breaks the
cell with a run-up and a breath before the loop.
...
def main():
    ...
    # Template: backing only, one 4-bar pattern, 4 channels.
    template = build_xm('stage1 drums+bass', 4,
                        [build_pattern(64, 4, backing_events())],
                        backing_instruments)
    write('stage1_drums_bass.xm', template)
    ...
    for filename, module_name, tune in [
        ('stage1_theme_a.xm', 'stage1 theme A anthem', TUNE_A),
        ('stage1_theme_b.xm', 'stage1 theme B driver', TUNE_B),
    ]:
        patterns = []
        for first_bar in (0, 4):
            events = dict(backing_events())
