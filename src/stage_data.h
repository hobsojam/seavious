...
typedef struct {
    ...
    int lengthPx;
    UpgradeAward award;
    // Constant environmental push (px/s), applied to player movement and
    // in-flight torpedoes (see docs/game-design.md "Stage 3"). {0, 0} for
    // stages without one - compound-literal descriptors below leave it
    // unlisted rather than writing that zero out explicitly.
    Vector2 drift;
} StageDescriptor;
...
extern const StageSpawnEvent STAGE3_EVENTS[];
extern const int STAGE3_EVENT_COUNT;
extern const StageTerrainFootprint STAGE3_TERRAIN[];
extern const int STAGE3_TERRAIN_COUNT;
extern const StageTerrainHardpoint STAGE3_TERRAIN_HARDPOINTS[];
extern const int STAGE3_TERRAIN_HARDPOINT_COUNT;
extern const int STAGE3_LENGTH_PX;
